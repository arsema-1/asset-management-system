...
router.post('/', authenticate, async (req: Request, res: Response) => {
  ...
  if (!assignment_id) {
    badRequest(res, 'assignment_id is required');
    return;
  }
  ...
  try {
    ...
    const aaRes = await db.query(
      `SELECT aa.*, a.name AS asset_name, a.asset_tag, a.id AS asset_id
       FROM asset_assignments aa
       JOIN assets a ON aa.asset_id = a.id
       WHERE aa.id = $1 AND aa.status = 'active'`,
      [assignment_id]
    );

    if (!aaRes.rows[0]) {
      notFound(res, 'Active assignment not found');
      return;
    }
